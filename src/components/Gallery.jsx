...
const destinations = [
  ...
  },
  {
    id: 'ella',
    name: 'Ella',
    coordinates: { x: 55, y: 70 }, // Position on custom SVG Map
    date: 'August 2025',
    story: 'Waking up to the mist rolling over Ella Rock and walking along the historic Nine Arch Bridge. A sanctuary of endless green valleys and cool mountain breezes.',
    highlights: ['Nine Arch Bridge hike', 'Ella Rock climb', 'Diyaluma Falls bath'],
    category: 'Mountains',
    image: 'https://images.unsplash.com/photo-1546708973-b339540b5162?auto=format&fit=crop&q=80&w=800' // Nine Arch Bridge, Ella
  },
  {
    id: 'sigiriya',
    name: 'Sigiriya',
    ...
    highlights: ['Climbing Lion Rock', 'Pidurangala sunrise', 'Ancient frescoes'],
    category: 'Cultural places',
    image: 'https://images.unsplash.com/photo-1588598126715-d91f24d7732a?auto=format&fit=crop&q=80&w=800' // Sigiriya
  },
  {
    id: 'nuwara-eliya',
    name: 'Nuwara Eliya',
    ...
    highlights: ['Gregory Lake boating', 'Tea factory tasting', 'Post office architecture'],
    category: 'Hiking',
    image: 'https://images.unsplash.com/photo-1563189336-1e63a1fb49de?auto=format&fit=crop&q=80&w=800' // Nuwara Eliya Tea Estates
  },
  {
    id: 'mirissa',
    name: 'Mirissa',
    ...
    highlights: ['Coconut Tree Hill sunset', 'Whale watching', 'Secret Beach snorkeling'],
    category: 'Beaches',
    image: 'https://images.unsplash.com/photo-1544735716-392fe2489ffa?auto=format&fit=crop&q=80&w=800' // Mirissa / Sri Lanka coast
  },
  {
    id: 'horton-plains',
    name: 'Horton Plains',
    ...
    highlights: ["World's End precipice", 'Baker’s Falls hike', 'Cloud forest biodiversity'],
    category: 'Camping',
    image: 'https://images.unsplash.com/photo-1627050414349-f027fc691238?auto=format&fit=crop&q=80&w=800' // Horton Plains/waterfall
  },
  {
    id: 'sinharaja',
    name: 'Sinharaja Forest',
    ...
    highlights: ['Endemic bird watching', 'Rainforest canopy walk', 'Natural pool swimming'],
    category: 'Forests',
    image: 'https://images.unsplash.com/photo-1448375240586-882707db888b?auto=format&fit=crop&q=80&w=800' // Deep Rainforest
  },
  {
    id: 'adams-peak',
    name: 'Adam’s Peak',
    ...
    highlights: ['Night climbing trail', 'Sunrise cloud sea', 'Sacred footprint temple'],
    category: 'Nature photography',
    image: 'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&q=80&w=800' // Sunrise Mountain
  }
];
...
const getEventTitle = (filename) => {
  const name = filename.toLowerCase();
  if (name.includes('earth summit2024') || name.includes('earths summit2024')) return 'Earth Summit 2024';
  if (name.includes('earth summit2025')) return 'Earth Summit 2025';
  return getEventCategory(filename);
};
...
const eventPhotos = Object.entries(rawGalleryImages).map(([path, url]) => {
  ...
  return {
    url,
    title: getEventTitle(filename),
    cat: getEventCategory(filename)
  };
});
...
export default function Gallery() {
  ...
  const eventCategories = ['Beach Cleanup', 'Earth Summit', 'Environmental Day', 'Me & Nature', 'Medical Camp', 'Sports', 'Yoga'];
  const categories = ['All', ...eventCategories, 'Mountains', 'Beaches', 'Forests', 'Camping', 'Nature photography', 'Hiking', 'Cultural places'];
      ...
      <div className="section-container">
        ...
        <div className="interactive-adventure-container glass-panel">
          ...
          <div className="map-column">
            ...
            <div className="map-canvas-container">
              {/* Styled Abstract SVG Map of Sri Lanka */}
              <svg viewBox="0 0 100 100" className="sri-lanka-svg">
                {/* Abstract Sri Lanka Outline */}
                <path 
                  d="M50,15 C55,17 58,22 60,28 C62,35 65,42 62,50 C59,58 57,66 54,74 C51,82 48,90 40,94 C38,91 36,87 35,84 C34,79 38,75 37,70 C36,65 30,62 31,56 C32,50 37,45 36,38 C35,31 38,25 42,20 Z" 
                  className="map-island-path"
                />
                
                {/* SVG Route Paths between pins */}
                <polyline 
                  points="50,40 46,60 50,62 40,66 48,68 55,70 38,80 42,92" 
                ...
                {destinations.map((dest) => (
                  <g 
                    key={dest.id}
                    className={`map-pin-group ${selectedDest.id === dest.id ? 'active' : ''}`}
                  ...
                  >
                    <circle 
                      ...
                      r="4" 
                      className="pin-pulse-circle" 
                    />
                    <circle 
                      cx={dest.coordinates.x} 
                      cy={dest.coordinates.y} 
                      r="2" 
                      className="pin-core-circle" 
                    />
                    {/* Floating label */}
                    <text 
                      x={dest.coordinates.x} 
                      y={dest.coordinates.y - 5} 
          ...
          <motion.div className="masonry-grid" layout>
            <AnimatePresence>
              {filteredPhotos.map((photo, idx) => (
                ...
                >
                  <img src={photo.url} alt={photo.title} className="masonry-image" />
                  <div className="masonry-overlay">
                    <ZoomIn className="zoom-icon" size={24} />
                    <div className="masonry-info">
                      <span className="masonry-cat">{photo.cat}</span>
        ...
        {lightboxImg && (
          ...
          >
            ...
            <motion.div 
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <img src={lightboxImg.url} alt={lightboxImg.title} className="lightbox-large-image" />
              <div className="lightbox-caption">
                <span className="lightbox-cat">{lightboxImg.cat}</span>
